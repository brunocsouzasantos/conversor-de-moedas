...
const getErrorMessage = errorType => ({
  ...
  'malformed-request': 'O endpoint do seu request precisa serguir a estrutura a seguir: https://v6.exchangerate-api.com/v6/YOUR-API-KEY/latest/USD',
  'invalid-key': 'A chave da API não é válida.',
  'quota-reached': 'Sua conta alcançou o limite de requests permitidos em seu plano atual.',
  'not-available-on-plan': 'Seu plano atuao não permite este tipo de request.'
})[errorType] || 'Não foi possível obter as informações.'

const fetchExchangeRate = async url => {
  try {
    const response =  await fetch(url)
    /* Vou comentar essa parte! Quando se tenta acessa um endpoint com moeda inesixtente, que seria 'unsopported-code
      na verdade a api retorna 404 e o response.ok é false, caindo sempre condicional.
      if (!response.ok) {
...
const getMutipliedExchangeRate = conversion_rates => {
...
const showUpdateRates = ({ conversion_rates }) => {
  convertedValueEl.textContent = getMutipliedExchangeRate(conversion_rates)
